    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `En parallèle du Front-end, j’ai également contribué à la qualité du Back-end en participant à la mise en place de plusieurs tests avec NUnit. J’ai notamment travaillé sur des tests unitaires et des tests d’intégration permettant de vérifier certains traitements critiques ainsi que les échanges de données entre les différents modules de l’application.`,
          `Cette double implication Front-end et Back-end m’a obligée à adapter continuellement ma manière de travailler. Passer de problématiques liées à l’expérience utilisateur à des sujets plus techniques autour de la validation des traitements m’a permis de développer une vision plus globale du fonctionnement de l’application.`,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `Enfin, le fait de participer également aux tests Back-end m’a permis d’élargir ma compréhension du cycle complet de développement d’une application et de mieux percevoir les interactions entre les différentes couches techniques du projet.`,
          `**Pour l’entreprise**`,
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `Après cette première phase de réflexion produit, je suis intervenue sur le Back-end avec ASP.NET Core 8 afin de développer plusieurs endpoints API permettant de centraliser et redistribuer les données nécessaires au Front-end.`,
          ...
          `En parallèle du Back-end, j’ai également participé au développement Front-end avec React 18 et Material UI. J’ai conçu plusieurs composants réutilisables afin de faciliter l’affichage dynamique des widgets et garantir une interface responsive adaptée aux différents formats d’écran.`,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `Le travail réalisé autour du cache mémoire m’a particulièrement marquée, car j’ai pu observer directement l’effet d’une optimisation Back-end sur la fluidité ressentie côté utilisateur. Cela a profondément changé ma manière d’aborder les problématiques de performance.`,
      ...
      },
      {
        title: 'Les acteurs',
        paragraphs: [
          `Le développement de CityBoard reposait sur une équipe de sept développeurs fullstack travaillant selon une organisation Agile. Les tâches étaient réparties entre les sujets Front-end, Back-end et intégration selon les priorités définies au cours des différents cycles de développement.`,
          `De mon côté, j’ai surtout collaboré avec les développeurs intervenant sur le Back-end et le Front-end afin de garantir que les données récupérées depuis les API externes pouvaient être affichées correctement dans les widgets. Lorsque je travaillais sur l’intégration d’un service ou sur la mise en cache des données, je devais régulièrement vérifier avec l’équipe que la solution restait compatible avec les besoins d’affichage, les performances attendues et les contraintes de personnalisation utilisateur.`,
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `L’un des enjeux principaux du projet concernait également la cohérence des calculs liés aux livraisons. Le prix proposé devait rester logique en fonction de plusieurs paramètres comme la distance, le poids du colis ou le trajet du transporteur. Il fallait donc assurer une communication fiable entre l’application mobile, le back-end et la base de données afin d’éviter les incohérences dans les informations affichées aux utilisateurs.`,
        ],
      },
      {
        title: 'Les étapes',
        paragraphs: [
          ...
          `En parallèle du développement mobile, j’ai aussi travaillé sur la structuration des données et la logique back-end en C# (.NET). J’ai contribué à l’organisation de la base SQL en définissant plusieurs relations entre les utilisateurs, les trajets, les colis et les messages afin de garantir la cohérence des échanges au sein de la plateforme.`,
          `Cette partie du projet m’a notamment amenée à intervenir sur les traitements liés aux calculs de prix et à la gestion des données géographiques. L’objectif était d’assurer une communication fiable entre l’application Flutter, les services back-end et la base de données tout au long du cycle de réservation et de livraison.`,
          ...
          `Une difficulté concrète que j’ai rencontrée concernait la cohérence des informations affichées entre le parcours de dépôt de colis et les données utilisées pour le calcul du prix. Certaines valeurs, comme le poids du colis ou la distance estimée, devaient rester synchronisées entre l’interface mobile, le back-end et la base de données. J’ai donc dû vérifier le parcours complet, depuis la saisie utilisateur jusqu’au traitement côté serveur, afin d’identifier les points où une incohérence pouvait apparaître. Cette situation m’a permis de mieux comprendre l’importance de tester une fonctionnalité non seulement écran par écran, mais aussi comme un scénario utilisateur complet.`,
        ],
      },
      {
        title: 'Les résultats',
